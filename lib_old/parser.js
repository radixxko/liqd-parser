...
const escapedRE = /\\(u\{([0-9A-Fa-f]+)\}|u([0-9A-Fa-f]{4})|x([0-9A-Fa-f]{2})|([1-7][0-7]{0,2}|[0-7]{2,3})|(['"tbrnfv0\\]))|\\U([0-9A-Fa-f]{8})/g;
const commonEscapes =
{
    '0': '\0',
    'b': '\b',
...
{
	return string.replace( escapedRE, ( _, __, varHex, longHex, shortHex, octal, specialCharacter, python ) =>
	{
...
module.exports = class Parser
{
	...
	parse( source )
	{
		...
		if( parsed_source === false || parsed_source.source.length )
		{
			...
			let max_parsed_position = source.length - Math.min( source.length, debug.remaining );
